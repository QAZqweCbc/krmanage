...
/// 获取当前版本信息
pub async fn get_version_info(State(state): State<AdminState>) -> impl IntoResponse {
    Json(state.service.get_version_info())
}

/// POST /api/admin/version/check
/// 检查是否有新版本
pub async fn check_update(State(state): State<AdminState>) -> impl IntoResponse {
    match state.service.check_update().await {
        Ok(response) => Json(response).into_response(),
        Err(e) => (e.status_code(), Json(e.into_response())).into_response(),
    }
}
