...
struct CredentialEntry {
    ...
    /// 是否需要刷新 Token（运行态，不写入 credentials.json）
    needs_refresh: bool,
    /// 短冷却截止时间（运行态，不写入 credentials.json）
    cooldown_until: Option<DateTime<Utc>>,
    /// 额度恢复时间（运行态，不写入 credentials.json）
    quota_recovery_at: Option<DateTime<Utc>>,
    /// 当前正在使用该凭据的请求数
    active_requests: u32,
    /// 最近一次错误 HTTP 状态码
...
pub struct CredentialEntrySnapshot {
    ...
    /// 额度恢复时间
    pub quota_recovery_at: Option<String>,
    /// 当前活跃请求数
...
impl MultiTokenManager {
    fn is_entry_selectable(
        entry: &CredentialEntry,
        now: DateTime<Utc>,
        model: Option<&str>,
    ) -> bool {
        if entry.disabled || entry.needs_refresh {
            return false;
        }
        if entry
            .cooldown_until
    ...
    fn clear_expired_transient_state(entry: &mut CredentialEntry, now: DateTime<Utc>) {
        if entry
            .cooldown_until
            .as_ref()
            .map(|until| *until <= now)
    ...
    fn set_last_error(
        ...
        entry.last_error_status = status;
        entry.last_error_message = Some(message.into());
        entry.last_error_at = Some(Utc::now().to_rfc3339());
        entry.last_used_at = Some(Utc::now().to_rfc3339());
    }

    fn clear_transient_fields(entry: &mut CredentialEntry) {
        entry.needs_refresh = false;
        entry.cooldown_until = None;
        entry.quota_recovery_at = None;
        entry.last_error_status = None;
        entry.last_error_message = None;
        entry.last_error_at = None;
    }

    fn next_month_quota_recovery_at(now: DateTime<Utc>) -> DateTime<Utc> {
    ...
    fn increment_active_requests(&self, id: u64) {
    ...
    /// 释放一次请求占用，调用方应在成功、失败、提前返回前都调用。
    pub fn release_context(&self, id: u64) {
        let mut entries = self.entries.lock();
        if let Some(entry) = entries.iter_mut().find(|e| e.id == id) {
    ...
    /// 标记凭据需要刷新，并临时从选择池移除。
    pub fn mark_needs_refresh(&self, id: u64, status: u16, message: impl Into<String>) -> bool {
        let mut entries = self.entries.lock();
        if let Some(index) = entries.iter().position(|e| e.id == id) {
            if entries[index].disabled {
                return entries
                    .iter()
                    .any(|e| Self::is_entry_selectable(e, Utc::now(), None));
            }
            let entry = &mut entries[index];
            entry.needs_refresh = true;
            entry.failure_count = 0;
            Self::set_last_error(entry, Some(status), message);
        }
        drop(entries);
        self.switch_to_next()
    }

    /// 标记短冷却，常用于 429 或连续瞬态错误。
    pub fn mark_cooldown(
        ...
        if let Some(entry) = entries.iter_mut().find(|e| e.id == id) {
            if !entry.disabled {
                entry.cooldown_until = Some(Utc::now() + duration);
                Self::set_last_error(entry, status, message);
            }
        }
        drop(entries);
        self.switch_to_next()
    ...
    pub fn mark_quota_recovery(
        ...
        if let Some(entry) = entries.iter_mut().find(|e| e.id == id) {
            if !entry.disabled {
                entry.quota_recovery_at = Some(recovery_at);
                entry.failure_count = 0;
                Self::set_last_error(entry, status, message);
            }
        }
    ...
    /// 标记不可刷新恢复的权限错误，并禁用该凭据。
    pub fn mark_forbidden(&self, id: u64, status: u16, message: impl Into<String>) -> bool {
        let mut entries = self.entries.lock();
        if let Some(entry) = entries.iter_mut().find(|e| e.id == id) {
            entry.disabled = true;
    ...
    pub fn report_transient_failure(
        ...
        let should_cooldown = {
            ...
            Self::set_last_error(entry, status, message.clone());
        ...
        if should_cooldown {
            self.mark_cooldown(
                id,
                Duration::seconds(DEFAULT_RATE_LIMIT_COOLDOWN_SECS),
                status,
                message,
            )
        } else {
            self.has_switch_target(None, None)
        }
    }

    /// 清理凭据临时状态。
    pub fn clear_transient_state(&self, id: u64) -> anyhow::Result<()> {
        let mut entries = self.entries.lock();
        let entry = entries
            .iter_mut()
            .find(|e| e.id == id)
            .ok_or_else(|| anyhow::anyhow!("凭据不存在: {}", id))?;
        Self::clear_transient_fields(entry);
        Ok(())
    }

    /// 是否存在可切换目标。
    pub fn has_switch_target(&self, exclude_id: Option<u64>, model: Option<&str>) -> bool {
        let now = Utc::now();
        let entries = self.entries.lock();
        entries
            .iter()
            .any(|e| Some(e.id) != exclude_id && Self::is_entry_selectable(e, now, model))
    }
    ...
    pub fn available_count(&self) -> usize {
        ...
        self.entries
            .lock()
            .iter()
            .filter(|e| Self::is_entry_selectable(e, now, None))
            .count()
    }
    ...
    fn select_next_credential(&self, model: Option<&str>) -> Option<(u64, KiroCredentials)> {
        ...
        for entry in entries.iter_mut() {
            Self::clear_expired_transient_state(entry, now);
        }

        // 过滤当前可调度的凭据
        let available: Vec<_> = entries
            .iter()
            .filter(|e| Self::is_entry_selectable(e, now, model))
            .collect();
    ...
    pub async fn acquire_context(&self, model: Option<&str>) -> anyhow::Result<CallContext> {
        ...
        loop {
            ...
            let (id, credentials) = {
                {
                    // 每次按当前策略选择可调度凭据，避免 priority 模式粘住临时切换后的账号。
                    ...
                    if let Some((new_id, new_creds)) = best {
                        ...
                        *current_id = new_id;
                        ...
                        let available = entries
                            .iter()
                            .filter(|e| Self::is_entry_selectable(e, Utc::now(), model))
                            .count();
                        if total > 0 && entries.iter().all(|e| e.disabled) {
                            anyhow::bail!("所有凭据均已禁用（0/{}）", total);
                        }
                        anyhow::bail!(
                            "所有凭据均暂不可用（可调度: {}/{}，可能原因：禁用、需刷新、冷却中、额度未恢复或不支持模型）",
                            available,
                            total
                        );
                    }
            ...
            match self.try_ensure_token(id, &credentials).await {
                Ok(ctx) => {
                    self.increment_active_requests(ctx.id);
                    return Ok(ctx);
                }
                Err(e) => {
    ...
    pub fn report_failure(&self, id: u64) -> bool {
        let result = {
            ...
            let Some(index) = entries.iter().position(|e| e.id == id) else {
                return entries
                    .iter()
                    .any(|e| Self::is_entry_selectable(e, now, None));
            };
    ...
    /// - 设置额度恢复时间（默认下月 1 日 UTC 00:00）
    /// - 恢复时间前跳过该凭据
    /// - 返回是否还有可调度凭据
    pub fn report_quota_exhausted(&self, id: u64) -> bool {
        let recovery_at = Self::next_month_quota_recovery_at(Utc::now());
        let result = self.mark_quota_recovery(id, recovery_at, Some(402), "QuotaExceeded");
        self.save_stats_debounced();
        result
    }
    ...
    pub fn switch_to_next(&self) -> bool {
        ...
        if let Some(next) = entries
            .iter()
            .filter(|e| e.id != *current_id && Self::is_entry_selectable(e, now, None))
            .min_by_key(|e| e.credentials.priority)
        {
            *current_id = next.id;
            tracing::info!(
            ...
            entries
                .iter()
                .any(|e| e.id == *current_id && Self::is_entry_selectable(e, now, None))
        }
    }
        ...
        ManagerSnapshot {
            entries: entries
                ...
                .map(|e| CredentialEntrySnapshot {
                    ...
                    quota_recovery_at: e.quota_recovery_at.as_ref().map(|t| t.to_rfc3339()),
                    active_requests: e.active_requests,
                    last_error_status: e.last_error_status,
                    last_error_message: e.last_error_message.clone(),
                    last_error_at: e.last_error_at.clone(),
    ...
    pub fn set_disabled(&self, id: u64, disabled: bool) -> anyhow::Result<()> {
        {
            ...
            if !disabled {
                ...
                Self::set_last_error(entry, None, "ManualDisabled");
            }
        }
        // 持久化更改
    ...
    pub async fn force_refresh_token_for(&self, id: u64) -> anyhow::Result<()> {
        ...
        // 无条件调用 refresh_token
        let effective_proxy = credentials.effective_proxy(self.proxy.as_ref());
        let new_creds = match refresh_token(&credentials, &self.config, effective_proxy.as_ref())
            .await
        {
            Ok(creds) => creds,
            Err(e) => {
                let invalid_refresh_token = e.downcast_ref::<RefreshTokenInvalidError>().is_some();
...
mod tests {
    ...
    fn api_key_credential(id: u64, priority: u32) -> KiroCredentials {
        KiroCredentials {
            id: Some(id),
            kiro_api_key: Some(format!("ksk_test_key_{}", id)),
    ...
    fn manager_with_api_keys() -> MultiTokenManager {
        MultiTokenManager::new(
            Config::default(),
            vec![api_key_credential(1, 0), api_key_credential(2, 1)],
            None,
            None,
            false,
        )
    ...
    async fn test_needs_refresh_skips_account_and_reset_clears_state() {
        ...
        assert!(manager.mark_needs_refresh(1, 401, "expired token"));
        assert!(manager.has_switch_target(Some(1), None));

        let ctx = manager.acquire_context(None).await.unwrap();
        assert_eq!(ctx.id, 2);
        manager.release_context(ctx.id);
        ...
        assert_eq!(first.last_error_status, Some(401));
        ...
        let first = manager
            ...
            .unwrap();
        assert!(!first.needs_refresh);
        assert!(first.last_error_status.is_none());
    }

    #[tokio::test]
    async fn test_cooldown_skips_account_until_recovered() {
        let manager = manager_with_api_keys();

        assert!(manager.mark_cooldown(1, Duration::seconds(60), Some(429), "rate limited"));
        let ctx = manager.acquire_context(None).await.unwrap();
        assert_eq!(ctx.id, 2);
        manager.release_context(ctx.id);

        {
            ...
            first.cooldown_until = Some(Utc::now() - Duration::seconds(1));
        }
    ...
    #[tokio::test]
    async fn test_quota_recovery_skips_account() {
        let manager = manager_with_api_keys();

        let recovery_at = Utc::now() + Duration::days(1);
    ...
    #[tokio::test]
    async fn test_active_requests_increment_and_release() {
        let manager = manager_with_api_keys();
    ...
    #[tokio::test]
    async fn test_multi_token_manager_quota_recovery_is_not_auto_recovered_before_time() {
        let config = Config::default();
        let cred1 = KiroCredentials::default();
        let cred2 = KiroCredentials::default();
